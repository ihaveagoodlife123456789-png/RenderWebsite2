    ...
    return (
        <div className="size-full bg-[url('/snowy-village-5120x2880-20406.jpg')] bg-no-repeat bg-cover flex justify-center items-center" initial={{scale: 0, y: -20}} animate={{scale: 1, y: 0}} transition={{duration: .1}}>
            ...
            {isSubmitSuccessful ? <Link to='/' className="absolute bottom-10"><motion.div className="text-white text-[18px] border-[3px] border-blue-800 font-semibold bg-blue-600/30 size-fit py-[6px] px-[4px] rounded-[6px]" initial={{scale: 0, y: -20}} animate={{scale: 1}} transition={{duration: .1}} whileHover={{scale: 1.06}}>Go home</motion.div></Link> : null}
                ...
                <motion.div className="absolute w-[94%] h-[42%] left-[7%] top-[6%] bg-slate-950/70 border-mist-950/90 border-[3px] size-[20px] font-semibold flex flex-col justify-center items-center gap-[45px] z-1" variants={variants} initial={{scale: 0, x: '-4.5%', y: '3%'}} animate={previewTemplate ? 'on' : 'off'}><div className="size-fit text-slate-300 flex justify-center gap-[38px] text-[35px] font-semibold"><h2>1234</h2><h2>John Doe</h2><h2 className="text-[goldenrod]">Welcome!</h2></div><div className="size-fit text-slate-300 flex justify-center gap-[38px]"><h3>Line Id</h3><h3>Name</h3><h3>Color: Goldenrod</h3></div><img src="/icons8-x-90.png" className="absolute right-6 top-6 h-[42px] w-[42px] scrollbar-none" variants={variants} initial={{opacity: 0}} animate={colorTemplate ? 'visible' : null} transition={{duration: .5}} onClick={() => previewSchema()}/></motion.div>
                ...
                <motion.div className="text-white absolute top-[12%] left-[54%] w-[240px] h-[180px] flex flex-col justify-center items-center gap-[20px] bg-slate-950/70 border-gray-900" variants={variants} initial={{scale: 0}} animate={emailTemplate ? 'on' : 'off'}><h2 className="relative left-[32%] font-bold size-[28px] text-emerald-600 w-full">Send email?</h2><div className="text-sky-500 flex justify-center items-center gap-[20px] font-semibold"><motion.h3 variants={variants} initial={{scale: 1}} onClick={() => emailBooleanOnClick()}>{emailChoose === false ? <motion.div layoutId="tab-indicator" transition={{layout: {duration: .5}}} className=" border-b-[2px] border-blue-300">Yes</motion.div> : <div onClick={() => setEmailChoose(!emailChoose)}>Yes</div>}</motion.h3><motion.h3 variants={variants} initial={{scale: 1}} onClick={() => emailBooleanOnClick()}>{emailChoose === true ? <motion.div layoutId="tab-indicator" transition={{layout: {duration: .5}}} className="border-b-[2px] border-blue-300">No thanks</motion.div> : <div onClick={() => setEmailChoose(!emailChoose)}>No thanks</div>}</motion.h3></div><img src="/icons8-x-90.png" className="absolute right-2 top-2 h-[22px] w-[22px] scrollbar-none" variants={variants} initial={{opacity: 0}} animate={colorTemplate ? 'visible' : null} transition={{duration: .5}} onClick={() => emailSchema()}/></motion.div>
