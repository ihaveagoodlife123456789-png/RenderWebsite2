    ...
    return (
        <div className="size-full bg-[url('/snowy-village-5120x2880-20406.jpg')] bg-no-repeat bg-cover flex justify-center items-center" initial={{scale: 0, y: -20}} animate={{scale: 1, y: 0}} transition={{duration: .1}}>
            ...
            {isSubmitSuccessful ? <Link to='/' className="absolute bottom-10"><motion.div className="text-white text-[18px] border-[3px] border-blue-800 font-semibold bg-blue-600/30 size-fit py-[6px] px-[4px] rounded-[6px]" initial={{scale: 0, y: -20}} animate={{scale: 1}} transition={{duration: .1}} whileHover={{scale: 1.06}}>Go home</motion.div></Link> : null}
                ...
                <motion.div className="absolute flex flex-col justify-center items-center left-[20%] top-[6%] border-slate-400/80 border-[3px] w-[90%] h-[90%] bg-slate-800/90 overflow-scroll" variants={variants} initial={{scale: 0, x: '-16.5%', y: '3%'}} animate={colorTemplate ? 'on' : 'false'}>{namedColors.map(color => <p className="text-blue-700">{color}</p>)}<img src="/icons8-x-90.png" className="absolute right-12 top-12 h-[72px] w-[72px]" variants={variants} initial={{opacity: 0}} animate={colorTemplate ? 'visible' : null} transition={{duration: .5}} onClick={() => colorSchema()}/></motion.div>
            </motion.div>
