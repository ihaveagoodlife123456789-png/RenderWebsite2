...
export function FormPage() { 
    return (
        <div className="size-full bg-[url('/snowy-village-5120x2880-20406.jpg')] bg-no-repeat bg-cover flex justify-center items-center" initial={{scale: 0, y: -20}} animate={{scale: 1, y: 0}} transition={{duration: .2}} onHover={{scale: 1.4}}>
            ...
            <motion className="flex flex-col align-items justify-content gap-5 font-semibold text-slate-200" >
                <h3>Name</h3>
                ...
                <motion.button className="size-fit text-slate-200 bg-blue-800 font-semibold text-[22px] rounded-[15px] py-[6px] px-[10px]" initial={{scale: 0}} onHover={{scale: 1.04}}>Submit</motion.button>
            </motion>
                <Link to='/'><motion.div className="text-white font-semibold absolute top-5 left-5 bg-blue-900/75 size-fit py-[4px] px-[2px] rounded-[10px]" initial={{scale: 0, y: -20}} animate={{scale: 1, y: 0}} transition={{duration: .2}} onHover={{scale: 1.04}}>Go home</motion.div></Link>
            </div>
