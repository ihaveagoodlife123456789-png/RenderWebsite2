...
export function GetUserProfile() {
    ...
    return (
        <div className="text-blue-800 text-[24px] font-bold size-full bg-[url('/snowy-village-5120x2880-20406.jpg')] bg-no-repeat bg-cover flex justify-center items-center">
            ...
            {userData ? (
                <>
                <h1 className="text-white font-bold">{userData.username ? <h2 className="text-orange-700"><h2 className="text-white">Username:</h2>{`${userData.username}`}</h2> : 'Please login first!'}</h1>
                    <h1 className="text-white font-bold">{userData.password ? <h2 className="text-orange-700"><h2 className="text-white">Password:</h2>{`${userData.password}`}</h2> : null}</h1>
